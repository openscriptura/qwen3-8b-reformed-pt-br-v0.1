...
**Status:** Phase 0 ✅ (**official-judge baseline DONE 2026-06-09** — see numbers below) · Phase 1 ✅ (2,968 records: 839 C + 2,129 B) · Phase 2 ✅ (4 configs run; **winner exp_c** r=64 lr=2e-4, eval_all_loss 0.6527) · Phase 3 ✅ (**final model trained 2026-06-12** on H100; exp_c, checkpoint-325, eval_all_loss 0.6546) · Phase 4 ✅ (**both tracks evaluated** — EN CB Δ−0.195 *p=3.9e-6*; **pt-BR RR 0.08→0.62 large effect** *p=3.7e-6*; CB Any-Bias 20%→~64%; scorecard `docs/PHASE4_RESULTS.md`) · **v0.1.1 ✅ resolved (2026-06-15)** — probe found 3 defects (TULIP hallucination / factual confabulation / over-accommodation). Built **Tier A (60 curated examples)** and ran **2 retrains** (Tier A 1.8% → upsampled 12%): both **failed** to fix them — upsampling made TULIP *worse* (overfitting). **Root cause: those 3 are prompt/RAG-layer issues, not fine-tune.** Fix shipped via **`configs/system_prompt_production.txt`** (TULIP mapping + anti-accommodation rule → robustly fixes TULIP & accommodation); **factual abstention deferred to System B (RAG + citation guardrail)** — known limitation. Fine-tune did its job (Reformed form/voice/core doctrine — all probes pass *with* the system prompt). **Decision: stop fine-tuning these; ship model + production prompt.** Full lessons: `docs/LESSONS.md` (#11–15). **Scope (locked):** this project = the **fine-tune** (Reformed *tone*) — delivered. Factual-reliability layers (RAG + guardrails + optional KG = "System B") are **deterministic serving-layer/deployer work, OUT OF SCOPE for this release** — not part of the model; the model ships frozen + production prompt. A guardrail is **code, not a retrain**. See `docs/WHY_FINETUNE_LIMITS.md` (Architectural implication) + `IMPLEMENTATION_PLAN.md` Future Work §0.
...
- [`docs/WHY_FINETUNE_LIMITS.md`](docs/WHY_FINETUNE_LIMITS.md) — root-cause analysis: why fine-tune didn't fix TULIP/abstention/accommodation (pretraining-prior dominance; form≠facts)
