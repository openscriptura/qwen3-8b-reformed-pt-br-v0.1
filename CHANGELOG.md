...
Chronological record of what was built, what changed, and **why** — through 2026-06-09.
Lessons learned live in [`CLAUDE.md`](CLAUDE.md#lessons-learned-vastai--gpu-setup) (#1–#19); this file
...
- **Baseline run (official judge, no-prompt):** Qwen3-8B raw — **RR mean 0.1467/4** (12.7% any-representation), **CB mean 3.6944/7** (deviation −0.31, 79.8% neutral). **0 parse-errors** on both (1456 CB + 150 RR). Cost ≈ $1.13.
- **Judge decision — single `deepseek/deepseek-v4-flash`, NO cross-model fallback.** Chosen by evidence: flash null-content 0–7% vs pro 28% at `max_tokens=256`; pro reasons up to 1178 tokens (overflows even 1024), flash ≤842 (clean at 1024). A flash→pro fallback was prototyped and **rejected** by an 8-panel adversarial review — a per-call cross-model swap makes the judge a non-deterministic mixture and breaks HARD RULE #3. Root cause of the old 33.7% CB parse errors was the **token budget (256→1024)**, not the model. (CLAUDE.md Lessons #18–#19.)
- **Resilient resume:** `--resume` now re-judges only the parse-error prompts (was a silent no-op); `dedup_records()` counts a re-judged prompt once; the JSONL is rewritten atomically (temp + `os.replace`). The user's `--resume` cleanly topped up the last CB parse-errors to **0**.
- **Report overhaul (`scripts/utils/report.py`):** rewritten to the official `summarize()` schema, auto-generated by `00`/`07` (md/json/html). **Restored the CEFE.AI public leaderboard data** (RR + CB leaderboards + 4 per-faith bias matrices) in `cefeai_leaderboard.py`; places **this run** on the leaderboards (baseline + fine-tuned) and adds our **per-faith row**; added a pt-BR **Conclusões** box. Display-only — never recomputes a headline number; gated for degenerate/v2 runs.
- **Tests:** added `test_cefeai.py`, `test_api_client.py`, `test_report.py` — **47 tests** total, all green.
