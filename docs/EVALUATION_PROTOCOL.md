...
| Validation (M9) | **Gates the absolute numbers when using flash:** human-label a ~50-item sample and report judge↔human agreement with **quadratic-weighted Cohen's κ** (ordinal scale); require ~substantial agreement (κ ≳ 0.6) before citing absolutes. | Justifies the weaker judge empirically; flags a miscalibrated judge. Also inspect per-tradition judge behavior (CB) for judge bias. |
